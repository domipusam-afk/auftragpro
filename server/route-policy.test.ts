...
const superAdminRoutesSourcePath = path.resolve(process.cwd(), "server/super-admin/routes.ts");
const routesSource = fs.readFileSync(routesSourcePath, "utf8");
const superAdminRoutesSource = fs.readFileSync(superAdminRoutesSourcePath, "utf8");
const routeSources = {
  routes: routesSource,
  "super-admin": superAdminRoutesSource,
} as const;
...
function actualLegacyEnforcement(source: string): "unguarded" | "guarded" {
  return /\b(?:requireAuth|requireSuperAdmin|requireAdminSession|requirePermission|createRequirePermission)\s*[,)]/.test(source)
    ? "guarded"
    : "unguarded";
}
...
const SAMPLE_EXPECTATIONS: ReadonlyArray<{
  ...
  { fingerprint: "GET /api/dashboard/reminders", access: "permissions", permissions: ["auftraege_anzeigen"] },
  { fingerprint: "GET /api/dashboard/aufgaben", access: "permissions", permissions: ["ressourcen_aufgaben"] },
  { fingerprint: "GET /api/dashboard/ueberfaellige-rechnungen", access: "permissions", permissions: ["dashboard_finanzen"] },
  { fingerprint: "GET /api/dashboard/deckungsbeitrag", access: "permissions", permissions: ["dashboard_finanzen"] },
  { fingerprint: "GET /api/dashboard/verlustrisiko", access: "permissions", permissions: ["dashboard_finanzen"] },
  { fingerprint: "PATCH /api/nachkalkulation/:id/status", access: "permissions", permissions: ["kalkulation_nachkalkulation"] },
  { fingerprint: "GET /api/dashboard/offene-nachkalkulation", access: "permissions", permissions: ["kalkulation_nachkalkulation"] },
...
function run(): void {
  const registered = [...registeredRoutes(routesSource), ...registeredRoutes(superAdminRoutesSource)];
  ...
  assert.equal(registered.length, 262, "Die bestätigte Anzahl der Express-Routen darf nicht unbemerkt abweichen.");
  ...
  const inlineGuardedLegacyRoutes = new Set([
    ...
    "POST /api/auftrag-kategorien",
    "DELETE /api/auftrag-kategorien/:id",
  ]);
  ...
  for (const expected of SAMPLE_EXPECTATIONS) {
    ...
    assert.equal(sourceAtLine(routeSources[policy.source || "routes"], policy.line).includes("app."), true, `Stichprobe ist keine aktuelle Route: ${expected.fingerprint}`);
  }
  ...
  assert.equal(discrepancies.length, 228, "Die erwarteten, noch nicht aktivierten Ziel-Gates müssen vollständig sichtbar bleiben.");
